...
char sdCommand(uint8_t pdrv, char cmd, unsigned int arg, unsigned int* resp)
{
    ...
    for (int f = 0; f < 3; f++) {
        ...
        if (token == 0xFF) {
            ...
            Serial.print("token error cmd=0x");
            Serial.print(cmd,HEX);
            Serial.print(" token=0x");
            Serial.println(cmd,token);
            break;
        }
    ...
    for (int f = 0; f < 3;) {
        ...
        if (!sdCommand(pdrv, READ_BLOCK_MULTIPLE, (s_cards[pdrv]->type == CARD_SDHC) ? sector : sector << 9, NULL)) {
            ...
            if (sdCommand(pdrv, STOP_TRANSMISSION, 0, NULL)) {
                Serial.println("!E*command failed");
                break;
            }
...
DSTATUS ff_sd_initialize(uint8_t pdrv)
{
    ...
    if (token == 0x5) {
        ...
        Serial.print("CRC_ON_OFF failed: ");
    ...
    if (sdTransaction(pdrv, SEND_IF_COND, 0x1AA, &resp) == 1) {
        if ((resp & 0xFFF) != 0x1AA) {
            Serial.print("SEND_IF_COND failed");
            Serial.println(resp & 0xFFF,HEX);
            goto unknown_card;
        }

        if (sdTransaction(pdrv, READ_OCR, 0, &resp) != 1 || !(resp & (1 << 20))) {
            Serial.print("READ_OCR failed:");
        ...
        if (token) {
            Serial.print("APP_OP_COND failed: ");
            Serial.println(token,HEX);
        ...
        if (!token) {
            ...
            if (token == 0x00) {
              ...
              Serial.print("SEND_OP_COND failed:");
              Serial.println(token,HEX);
                goto unknown_card;
            }
        }
...
bool sdcard_mount(uint8_t pdrv, const char* path)
{
    ...
    if (err == ESP_ERR_INVALID_STATE) {
        Serial.print("esp_vfs_fat_register failed 0x(");
    ...
    if (res != FR_OK) {
      Serial.print("!E*f_mount failed 0x");
